...
{\scriptsize
<<e2f3>>=
tbl <- interactions(refnet, species="9606", id="E2F3", provider=c("gerstein-2012", "BioGrid"))
dim(tbl)
@ 
}
...
{\scriptsize
<<mapper>>=
tbl.1 <- interactions(refnet, id="E2F3", provider="IntAct", species="9606")
dim(tbl.1)
idMapper <- IDMapper("9606")
tbl.1 <- addStandardNames(idMapper, tbl.1)
dim(tbl.1)
tbl.1[, c("A.common", "B.common", "A.canonical", "B.canonical", "type", "provider")]
@ 
}
...
{\scriptsize
<<mixedColumns>>=
tbl.2 <- interactions(refnet, id="ACO2", provider=c("gerstein-2012", "BioGrid"))
tbl.2 <- addStandardNames(idMapper, tbl.2)
sort(colnames(tbl.2))
@ 
}
...
<<xtab>>=
options(width=180)
tbl.info <- with(tbl, as.data.frame(table(detectionMethod, type, provider)))
...
<<dups>>=
  ...
  tbl.withDups <- detectDuplicateInteractions(tbl)
@ 
}
...
geneID <- unique(subset(tbl, A.common=="FZR1")$A.canonical)
...
providers <- intersect(unlist(providers(refnet), use.names=FALSE),
                       c("BIND", "BioGrid", "IntAct", "MINT",
                         "gerstein-2012"))
tbl <- interactions(refnet, species="9606", id="E2F3", 
                    provider=providers)
tbl <- addStandardNames(idMapper, tbl)
dim(tbl)
@ 
...
{\scriptsize
<<detectDups>>=
tbl <- detectDuplicateInteractions(tbl)
dupGroups <- sort(unique(tbl$dupGroup))
preferred.types <- c("direct interaction",
                     "physical association",
                     "transcription factor binding")
bestOfDups <- unlist(lapply(dupGroups, function(dupGroup) 
                         pickBestFromDupGroup(dupGroup, tbl, preferred.types)))

deleters <- which(is.na(bestOfDups))
if(length(deleters) > 0)
    bestOfDups <- bestOfDups[-deleters]
length(bestOfDups)
tbl.best <- tbl[bestOfDups,]
tbl.best[, c("A.common", "B.common", "type", "provider", "publicationID")]
@ 
}
